...
public class HandleMsgTask implements Runnable{
	 ...
	 * 消息处理
	 */
	private JSONArray handleMsg(Wechat wechat, JSONObject msgJson, WechatContact wechatContact) {
...
//				logger.info("系统消息...");
			...
			// 文本消息
				...
				if (fromUserName.equals(userName)) {
					...
					if(sendMsg.contains("@" + wechat.getUser().getString("NickName"))) {
						//回复
						JSONObject replyJson = TulingAPI.getReply(sendMsg.replace("@"+wechat.getUser().getString("NickName"),""),
								sendUserName.replace("@", "").substring(1, 10), wechat.getCity());
						if(null != replyJson) {
							String reply = TulingAPI.handleReply(replyJson);
							if(null != reply) {
				...
				} else if (content.contains("点歌@")) {
					String musicName = content.replace("点歌@", "").trim();
					this.sendMusic(wechat, musicName, msg.getString("FromUserName"));
				} else {
					logger.info("msg:[{}]:{}", nickName, content);
					// 回复消息
					JSONObject replyJson = TulingAPI.getReply(content, fromUserName.replace("@", "").substring(1, 10), wechat.getCity());
					if(null != replyJson) {
						String reply = TulingAPI.handleReply(replyJson);
						if(null != reply) {
							this.sendMsg(wechat, reply, fromUserName);
	...
	// 发送音乐
	private void sendMusic(Wechat wechat, String musicName, String toUserName) {
		String con = "";
		String url = wechat.getBase_uri() + "/webwxsendmsg?" + "lang=zh_CN" + "&pass_ticket=" + wechat.getPassTicket();
		JSONObject body = new JSONObject();
		String clientMsgId = DateKit.getCurrentUnixTime() + StringKit.getRandomNumber(5);
		...
		info.put("AppID", "wx8dd6ecd81906fd84");
		info.put("Type", 0);
		msg.put("AppInfo", info);
		msg.put("Content", con);
		msg.put("AppMsgType", "3");
		msg.put("Type", 49);
		msg.put("FileName", "让一切随风");
		msg.put("Url", "http://music.163.com/song/194594?userid=126150631");
		msg.put("FromUserName", wechat.getUser().getString("UserName"));
		msg.put("ToUserName", toUserName);
		msg.put("LocalID", clientMsgId);
		msg.put("ClientMsgId", clientMsgId);
